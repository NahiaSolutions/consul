class User < ActiveRecord::Base
    ...
    has_one :houses_administrator
    has_one :volunteerings_administrator
    has_one :moderator
    has_one :valuator
    has_one :manager
    ...
    #Validadación de la cédula activar cuando se quiera validar desde el modelo
    ...
    #Campos obligatorios (ojo en los seeds deben estar presentes estos campos sino da error)
    validates :username, :presence => true
    validates :userlastname, :presence => true
    validates :document_type, :presence => true
    validates :document_number, :presence => true, uniqueness: { scope: :document_type }
    validates :date_of_birth, :presence => true
    validates :email, :presence => true
    validates :user_parish, :presence => true
    validates :user_neighborhood, :presence => true
    ...
    def self.gender_options
      #[ ["Hombre", '1'], ["Mujer", '2'], ["LGBTI", '3'] ]  #Si quiero sacar los indices
    ...
    #etnias disponibles
    def self.ethnic_options
      [ ["Afroecuatoriano"], ["Mestizo"], ["Montubio"], ["Indígena"], ["Blanco"], ["Otros"] ]
    end
    #etnias disponibles
    def self.studies_options
      [ ["Básico"], ["Bachillerato"], ["Tercer Nivel"], ["Cuarto Nivel"], ["Ninguno"] ]
    end
    ...
    #Admin casa somos
    def houses_administrator?
      houses_administrator.present?
    end

    #Admins voluntariado
    def volunteerings_administrator?
      volunteerings_administrator.present?
    end
    ...
    #Validar cédula con el webService SOAP del municipio en la vista
    def self.valid_document_number(document_number, type)
      soap_header = { "mdmq:Cabecera" => { "mdmq:UserName" => '30', "mdmq:Password" => '123456' }}
      wsdl =  'http://172.22.8.108/MDMQ_WS_PERSONAS2012/Presentacion/PersonaSQL.asmx?wsdl'
      message = {'mdmq:strIdentificacion' => document_number, 'mdmq:strTipoIdentificacion' => type}
      client = Savon.client(wsdl: wsdl, soap_header: soap_header, namespace_identifier: :mdmq, env_namespace: :soapenv)
      response = client.call(:fn_consulta_buscar_persona_completo, message: message)
      data = response.to_hash[:fn_consulta_buscar_persona_completo_response]
    end

    def self.valid_document_number_model(document_number, type)
      soap_header = { "mdmq:Cabecera" => { "mdmq:UserName" => '30', "mdmq:Password" => '123456' }}
      wsdl =  'http://172.22.8.108/MDMQ_WS_PERSONAS2012/Presentacion/PersonaSQL.asmx?wsdl'
      message = {'mdmq:strIdentificacion' => document_number, 'mdmq:strTipoIdentificacion' => type}
      client = Savon.client(wsdl: wsdl, soap_header: soap_header, namespace_identifier: :mdmq, env_namespace: :soapenv)
      response = client.call(:fn_consulta_buscar_persona_completo, message: message)
      data = response.to_hash[:fn_consulta_buscar_persona_completo_response][:str_mensaje]
    end
    ...
    private
      ...
      #Validación del numero de cedula
      def validate_document_number
        if document_type == "Cédula"
          if User.valid_document_number_model(document_number, 'C') != "PROCESO OK"
            errors.add(:document_number, "Cédula no válida")
          end
        end
        if document_type == "Pasaporte"
          if User.valid_document_number_model(document_number, 'P') != "PROCESO OK"
            errors.add(:document_number, "Pasaporte no válido")
          end
        end
      end
